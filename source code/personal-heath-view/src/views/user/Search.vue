<template>
    <el-row>
        <el-row v-if="newsData.length === 0">
            <el-empty description="no related news"></el-empty>
        </el-row>
        <el-row v-else style="margin-top: 20px;">
            <el-col class="new-item" @click.native="newsItemClick(news)" :span="4" :key="index"
                ...
                <div style="font-size: 12px;">
                    ...
                    <span style="margin-left: 10px;">published at {{ parseTime(news.createTime) }}</span>
                </div>
            </el-col>
        </el-row>
    </el-row>
    ...
    methods: {
        ...
        async fetchData() {
            try {
                ...
                console.error(`search news list load exception:${e}`);
            }
        },
    },
};
